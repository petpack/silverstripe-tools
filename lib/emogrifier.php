...
class Emogrifier {
    ...
    private $unprocessableAttributes = array('href', 'src');
    ...
    public function setCSS($css = '') { $this->css = "\n".$css; }
    ...
    public function handleReplacement($matches){
    		
        $fullMatch = $matches[0];
        if (!isset($this->templateReplacements[$fullMatch])){
            $this->templateReplacements[$fullMatch] = "_emogrifier_placeholder_".count($this->templateReplacements).'_="1"';
        }
        return $this->templateReplacements[$fullMatch];
    }

    // applies the CSS you submit to the html you submit. places the css inline
	public function emogrify() {
	    $body = $this->html;
	    $this->templateReplacements = array();
	    $this->unprocessableHTMLTags[] = $this->templateTag;
	    // process the CSS here, turning the CSS style blocks into inline css
	    if (count($this->unprocessableHTMLTags)) {
            $unprocessablePattern = array();
            foreach ($this->unprocessableHTMLTags as $tag){
            	$unprocessablePattern[] = "/<($tag)[^>]*>/i";
            }
            foreach ($this->unprocessableAttributes as $attribute){
            	$unprocessablePattern[] = '/'.$attribute.'\s*="[^"]*\$[^"]*"/i';
            }
            $body = preg_replace_callback($unprocessablePattern,array($this, 'handleReplacement'), $body);
	    }
		...
		if (count($this->templateReplacements)){
			$html = str_replace($this->templateReplacements, array_keys($this->templateReplacements), $html);
		}
		return $html;
